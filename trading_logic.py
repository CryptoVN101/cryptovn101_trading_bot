...
async def get_klines(symbol, interval, client=None, limit=1000):
    if not client:
    ...
    try:
        ...
        if client and not client.session.closed:  # Kiểm tra session của client
    ...
    async def initialize_watches():
        ...
        for i in range(0, len(watchlist), 5):
            ...
            for symbol in batch:
                ...
                tasks.append(get_klines(symbol, TIMEFRAME_M15, client))
                tasks.append(get_klines(symbol, TIMEFRAME_H1, client))
